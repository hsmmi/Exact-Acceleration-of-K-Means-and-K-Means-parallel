...
def read_dataset(file, atr):
    if type(atr) == int:
        with open(os.path.join(os.path.dirname(__file__), file), "r") as f:
            return list(
                map(lambda x: float(x.split(", ")[atr]), f.read().splitlines())
            )
    else:
        with open(os.path.join(os.path.dirname(__file__), file), "r") as f:
            return list(
                map(
                    lambda x: [
                        float(i) for i in (x.split(", ")[atr[0] : atr[1]])
                    ],
                    f.read().splitlines(),
                )
            )
...
def read_dataset_with_pandas(file: str, atr=None):
    colName = pd.read_csv(
        os.path.join(os.path.dirname(__file__), file), nrows=0
    ).columns
    if type(atr) == int:
        colName = [colName[atr]]
...
def read_dataset_to_X_and_y(
    ...
    feature = np.array(
        list(map(lambda x: x[range_feature[0] : range_feature[1]], data))
    )
    label = np.array(
        list(map(lambda x: x[range_label[0] : range_label[1]], data))
    )
...
def nparray_to_csv(
    file: str, input: np.ndarray, decimal: int | None = None
) -> None:
...
def generate_dataset(matrix_size: int, file: str = None) -> np.ndarray:
